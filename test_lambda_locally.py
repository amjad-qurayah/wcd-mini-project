...
def get_customer_id(TopCustomers):
    """
    Read customer_id_file and return a string list of customer_id
    """
    li=[]
    with open(TopCustomers,'rb') as f:
        data = json.load(f)
...
def lambda_handler(event, context):
    ...
    if 'Records' in event and len(event['Records']) > 0:
        ...
        if 's3' in s3_record and 'object' in s3_record['s3']:
            ...
            if 'key' in s3_object:
              ...
              if len(key_parts) > 1:
                ...
                raise ValueError("Invalid S3 key format: {}".format(s3_object['key']))
            else:
              raise ValueError("Invalid S3 event format: {}".format(s3_record))
        else:
           raise ValueError("Invalid event format: {}".format(event))
